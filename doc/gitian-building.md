...
- [Building Litecoin Core](#building-chaindev_latoken-core)
...
Clone the git repositories for chaindev_latoken and Gitian.
...
in the chaindev_latoken repository under 'Fetch and create inputs' to install sources which require
...
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the chaindev_latoken repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/chaindev_latoken/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/chaindev_latoken-project/chaindev_latoken
...
For example:
```bash
URL=https://github.com/thrasher-/chaindev_latoken.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit chaindev_latoken=${COMMIT} --url chaindev_latoken=${URL} ../chaindev_latoken/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit chaindev_latoken=${COMMIT} --url chaindev_latoken=${URL} ../chaindev_latoken/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit chaindev_latoken=${COMMIT} --url chaindev_latoken=${URL} ../chaindev_latoken/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the chaindev_latoken git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../chaindev_latoken/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/chaindev_latoken
SIGPATH=/some/root/path/chaindev_latoken-detached-sigs

./bin/gbuild --url chaindev_latoken=${BTCPATH},signature=${SIGPATH} ../chaindev_latoken/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/chaindev_latoken-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/chaindev_latoken-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/chaindev_latoken-osx-build.assert
```
...
[chaindev_latoken-project/gitian.sigs.ltc](https://github.com/chaindev_latoken-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
