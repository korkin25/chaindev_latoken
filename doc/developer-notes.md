...
Run with the -testnet option to run with "play chaindev_latokens" on the test network, if you
...
- BitcoinMiner : Generates chaindev_latokens (if wallet is enabled).
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:chaindev_latoken-project/chaindev_latoken.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `chaindev_latoken-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `chaindev_latoken-cli` and the GUI debug console use this table to determine how to
