Litecoin Core version 0.13.2 is now available from:
  ...
  <https://download.chaindev_latoken.org/chaindev_latoken-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/chaindev_latoken-project/chaindev_latoken/issues>
...
updates will be released anymore. Without security updates, using a chaindev_latoken
...
This allows running chaindev_latokend without having to do any manual configuration.
...
`chaindev_latoken.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /chaindev_latokenrpc>
        ...
        # optional bypass chaindev_latokend rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< chaindev_latokenrpc:<password>
    </Location>
...
Restarting chaindev_latokend will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in chaindev_latoken.conf
...
chaindev_latoken-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/chaindev_latoken-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `chaindev_latoken-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `chaindev_latoken-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of chaindev_latokens they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of chaindev_latokens being spent is exactly
...
- `chaindev_latoken-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `chaindev_latoken-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
