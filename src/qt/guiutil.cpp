...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("chaindev_latoken"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("chaindev_latoken://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "chaindev_latoken:");
    }
...
{
    QString ret = QString("chaindev_latoken:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "chaindev_latoken.desktop";
    return GetAutostartDir() / strprintf("chaindev_latoken-%s.lnk", chain);
}
