...
example /usr/src/chaindev_latoken, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/chaindev_latoken-project/chaindev_latoken.git
    sudo chmod -R a+rw chaindev_latoken
...
way. This will install to `c:\workspace\chaindev_latoken`, for example:

    make install DESTDIR=/mnt/c/workspace/chaindev_latoken
...
causes two of the chaindev_latoken executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the chaindev_latoken code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the chaindev_latoken source code).
